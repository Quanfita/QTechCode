...
from .apis.qr_code import get_qrcode, get_bg_qrcode, get_logo_qrcode, save_qrcode
from .apis.water_mark import marker
from .apis.common import GiteeImageCloud
...
def qr_code_view(request):
    if request.is_ajax() and request.method == "POST":
        data = request.POST
        text = data.get('text')
        save_dir = os.path.join(settings.MEDIA_ROOT, 'qrcode')
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        name = str(uuid1())+'.png'
        save_path = os.path.join(save_dir, name)
        save_qrcode(get_qrcode(text), save_path)
        _, base = GiteeImageCloud().encode_base64(save_path)
        res ={
            "data": base,
            "name": name
        }
        os.remove(save_path)
        return JsonResponse(res)
    return render(request, 'tool/qrcode.html')

@csrf_exempt
def watermarker_view(request):
    def handle_uploaded_file(save_f, f):
        with open(save_f, 'wb+') as destination:
    ...
    if request.is_ajax() and request.method == "POST":
        ...
        name = str(uuid1()) +'.'+ file_type
        upload_path = os.path.join(settings.MEDIA_ROOT, 'upload')
        path = os.path.join(upload_path, name)
        if not os.path.exists(upload_path):
            os.mkdir(upload_path)
        handle_uploaded_file(path, request.FILES['file'])
        save_dir = os.path.join(settings.MEDIA_ROOT, 'watermarker')
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_path = os.path.join(save_dir, 'new_'+name)
        marker(path, text, save_path)
        _, base = GiteeImageCloud().encode_base64(save_path)
        res ={
            "data": base,
            "name": name
        ...
        os.remove(path)
        return JsonResponse(res)
    return render(request, 'tool/watermarker.html')